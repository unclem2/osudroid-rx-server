...
def write_event(
    ...
    if event == "spectatorData":
        ...
        return
    with open(f"/srv/odrx_storage/dev/rooms/{id}.jsonl", "a") as f:
        dump_data = {
            "event": event,
            "data": data,
...
def read_room_log(id: int) -> list:
    with open(f"/srv/odrx_storage/dev/rooms/{id}.jsonl", "r") as f:
        room_data = []
...
def get_id() -> str:
    ...
    rooms = os.listdir("/srv/odrx_storage/dev/rooms")
    if len(rooms) == 0:
        return "1"
    else:
